...
import { Driver, getDriverStatusText, getDriverById } from '../services/DriverApi';
...
const DriverDetailDialog: React.FC<DriverDetailDialogProps> = ({
  ...
  const DialogComponent = standalone ? Box : Dialog;
  const dialogProps = standalone 
    ? { sx: { maxWidth: '1000px', margin: '0 auto', mt: 3 } }
    : { open, onClose: handleClose, maxWidth: "md", fullWidth: true };
  ...
  return (
    <DialogComponent {...dialogProps}>
      ...
      <DialogContent dividers={!standalone}>
        {loading ? (
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12}>
              <Box display="flex" alignItems="center" gap={2}>
                ...
                <Box>
                  <Typography variant="h5">{driver.fullName || 'Không có tên'}</Typography>
                  <Box display="flex" alignItems="center" gap={1} mt={0.5}>
                    <Chip 
                      ...
                      color={
                        ...
                        driver.status === 2 ? "error" :
                        driver.status === 3 ? "warning" : "default"
                      } 
                    />
                ...
                <CardContent>
                  ...
                  <Box display="flex" alignItems="center" gap={1}>
                    ...
                    <Typography variant="h6">
                      {driver.totalWorkingTime !== undefined ? formatTime(driver.totalWorkingTime) : 'N/A'}
                    </Typography>
                  </Box>
                </CardContent>
              </Card>
              ...
              <Card variant="outlined">
                <CardContent>
                  ...
                  <Box display="flex" alignItems="center" gap={1}>
                    ...
                    <Typography variant="h6">
                      {driver.currentWeekWorkingTime !== undefined ? formatTime(driver.currentWeekWorkingTime) : 'N/A'}
                    </Typography>
                  </Box>
                </CardContent>
              </Card>
            ...
            <Grid item xs={12}>
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} md={6}>
                  ...
                  <Typography variant="body1" gutterBottom>{driver.phoneNumber || 'Chưa cập nhật'}</Typography>
                  
                  <Typography variant="subtitle2">Ngày sinh</Typography>
                  <Typography variant="body1" gutterBottom>
                    {driver.dateOfBirth ? new Date(driver.dateOfBirth).toLocaleDateString('vi-VN') : 'Chưa cập nhật'}
                  </Typography>
                </Grid>
                ...
                <Grid item xs={12} md={6}>
                  ...
                  <Typography variant="body1" gutterBottom>
                    {driver.createdDate ? new Date(driver.createdDate).toLocaleString('vi-VN') : 'N/A'}
                  </Typography>
                  
                  <Typography variant="subtitle2">Cập nhật lần cuối</Typography>
                  <Typography variant="body1" gutterBottom>
                    {driver.modifiedDate ? new Date(driver.modifiedDate).toLocaleString('vi-VN') : 'Chưa cập nhật'}
                  </Typography>
                  
                  <Typography variant="subtitle2">Người tạo</Typography>
                  <Typography variant="body1" gutterBottom>{driver.createdBy || 'N/A'}</Typography>
                </Grid>
              </Grid>
            </Grid>
      ...
      <DialogActions sx={standalone ? { paddingX: 0, mt: 2 } : {}}>
        <Button onClick={handleClose}>{standalone ? 'Quay lại' : 'Đóng'}</Button>
      </DialogActions>
    </DialogComponent>
  );
};
