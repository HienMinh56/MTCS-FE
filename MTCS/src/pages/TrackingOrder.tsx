...
const ConnectionStatus = styled(Box)(
  ({
    theme,
    status,
  }: {
    theme: any;
    status: "connecting" | "connected" | "disconnected" | "receiving";
  }) => ({
    display: "flex",
    alignItems: "center",
...
const TrackingOrder: React.FC = () => {
  ...
  const [connectionStatus, setConnectionStatus] = useState<
    "connecting" | "connected" | "disconnected" | "receiving"
  >("disconnected");
  ...
  const handleTrackOrder = async () => {
    ...
    try {
      ...
      setTrackingData(response.data); // Get geocoding for locations - use first orderDetail
      const fetchLocations = async () => {
        try {
          const firstOrderDetail = response.data.orderDetails[0];
          const pickupCoords = await getGeocodeByAddress(
            firstOrderDetail.pickUpLocation
          );
          const deliveryCoords = await getGeocodeByAddress(
            firstOrderDetail.deliveryLocation
          );

          setLocations({
            pickup: {
              name: firstOrderDetail.pickUpLocation,
              coordinates: pickupCoords || undefined,
            },
            delivery: {
              name: firstOrderDetail.deliveryLocation,
      ...
      if (err.message === "ORDER_COMPLETED") {
        ...
        setError(
          "Không thể tìm thấy mã vận chuyển của bạn. Vui lòng kiểm tra xem mã có đã đúng hay không."
        );
      }
  ...
  const sortedStatuses = useMemo(() => {
    if (!trackingData?.orderDetails?.[0]?.trips?.[0]?.tripStatusHistories)
      return [];

    return [...trackingData.orderDetails[0].trips[0].tripStatusHistories].sort(
      (a, b) =>
  ...
  const handleStopTracking = () => {
    ...
    if (socketRef.current) {
      ...
      if (
        socketRef.current.state === signalR.HubConnectionState.Connected &&
        driverId
      ) {
        socketRef.current.invoke("Unsubscribe", driverId).catch((err) => {
  ...
  const connectSignalR = (userId: string) => {
    ...
    socketRef.current.on(
      "ReceiveLocation",
      (receivedUserId: string, latitude: number, longitude: number) => {
        console.log(`SignalR message received for user ${receivedUserId}:`, {
          latitude,
          longitude,
        });
    ...
    function attemptConnection() {
      ...
      socketRef
        ...
        .then(() => {
          console.log(
            `Successfully subscribed to location updates for driver: ${userId}`
          );
        })
        .catch((err) => {
          ...
          if (retryCount < maxRetries) {
            console.log(
              `Retry attempt ${retryCount} failed. Trying again in 3 seconds...`
            );
  ...
  const updateDriverLocation = (lat: number, lng: number) => {
    ...
    if (!driverMarkerRef.current) {
      ...
      el.style.backgroundImage =
        'url(\'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%233f51b5"><path d="M18.92 6.01C18.72 5.42 18.16 5 17.5 5h-11c-.66 0-1.21.42-1.42 1.01L3 12v8c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h12v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-8l-2.08-5.99zM6.5 16c-.83 0-1.5-.67-1.5-1.5S5.67 13 6.5 13s1.5.67 1.5 1.5S7.33 16 6.5 16zm11 0c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zM5 11l1.5-4.5h11L19 11H5z"/></svg>\')';
      ...
      driverMarkerRef.current.setPopup(
        new window.goongjs.Popup({ offset: 25 }).setHTML(
          `<h3>Vị trí tài xế</h3><p>Tên tài xế: ${driverName}</p>`
        )
      );
      ...
      fetch(directionsUrl)
        ...
        .then((data) => {
          ...
          if (
            ...
            console.log(
              "Got polyline:",
              polyline ? "Yes (length: " + polyline.length + ")" : "No"
            );
            ...
            if (decodedCoords.length > 0) {
              ...
              console.log(
                "Driver to delivery route added to map successfully!"
              );
            }
          } else {
  ...
  useEffect(() => {
    if (trackingData?.orderDetails?.[0]?.trips?.[0]?.driverId) {
      const firstTrip = trackingData.orderDetails[0].trips[0];
      const tripDriverId = firstTrip.driverId;
      const driverName = firstTrip.driver.fullName;
      ...
      if (locations.pickup?.coordinates && locations.delivery?.coordinates) {
        ...
        console.log(
          "Waiting for location coordinates before starting tracking:",
          locations.pickup?.coordinates ? "Pickup ready" : "Pickup not ready",
          locations.delivery?.coordinates
            ? "Delivery ready"
            : "Delivery not ready"
        );
      }
  ...
  useEffect(() => {
    ...
    const initMap = () => {
      ...
      const pickupCoords = [
        locations.pickup!.coordinates!.lng,
        locations.pickup!.coordinates!.lat,
      ];
      const deliveryCoords = [
        locations.delivery!.coordinates!.lng,
      ...
      map.on("load", () => {
        // Add pickup marker
        pickupMarkerRef.current = new window.goongjs.Marker({
          color: "#4CAF50",
        })
          .setLngLat(pickupCoords)
          .setPopup(
            new window.goongjs.Popup({ offset: 25 }).setHTML(
              `<h3>Nơi lấy hàng</h3><p>${locations.pickup!.name}</p>`
            )
          )
          .addTo(map);

        // Add delivery marker
        deliveryMarkerRef.current = new window.goongjs.Marker({
          color: "#F44336",
        })
          .setLngLat(deliveryCoords)
          .setPopup(
            new window.goongjs.Popup({ offset: 25 }).setHTML(
              `<h3>Nơi giao hàng</h3><p>${locations.delivery!.name}</p>`
            )
          )
          .addTo(map);
              ...
              if (decodedCoords.length > 0) {
                ...
                map.addSource("route", {
                  ...
                  data: {
                    ...
                    geometry: {
                      type: "LineString",
                      coordinates: decodedCoords.map((coord) => [
                        coord.lng,
                        coord.lat,
                      ]),
                    },
  ...
  const renderDriverVehicleDetails = () => {
    if (!trackingData?.orderDetails?.[0]?.trips?.[0]?.driver) return null;

    const { driver, tractor, trailer } = trackingData.orderDetails[0].trips[0];
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} md={6}>
              <Box sx={{ display: "flex", alignItems: "center" }}>
                ...
                />
                <Box>
                  <Typography variant="subtitle2">Đầu kéo</Typography>
                  <Typography variant="body2" color="text.secondary">
            ...
            <Grid item xs={12} md={6}>
              <Box sx={{ display: "flex", alignItems: "center" }}>
                ...
                />
                <Box>
                  <Typography variant="subtitle2">Rơ-móoc</Typography>
                  <Typography variant="body2" color="text.secondary">
      ...
      <MapControlsContainer>
        ...
        >
          <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>
            Trạng thái dò tìm vị trí tài xế
          </Typography>
        </Box>
      ...
      <Paper sx={{ p: { xs: 2, md: 4 }, borderRadius: 2, mb: 4 }}>
        ...
        <Box sx={{ display: "flex", mb: 4, gap: 1 }}>
          <TextField
            ...
            placeholder="e.g., DET20250603135c61"
            error={!!error}
            helperText={error}
            InputProps={{
              startAdornment: (
              ...
              <Grid item xs={12} md={5}>
                    ...
                    <Box sx={{ display: "flex", alignItems: "flex-start" }}>
                      ...
                      <Box>
                        ...
                        <Typography variant="body1" sx={{ fontWeight: 500 }}>
                          {trackingData.orderDetails[0].pickUpLocation}
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                          {dayjs(
                            trackingData.orderDetails[0].pickUpDate
                          ).format("MMM D, YYYY")}
                        </Typography>
                      </Box>
                    </Box>
                ...
                >
                  <CardContent>
                    <Box sx={{ display: "flex", alignItems: "flex-start" }}>
                      <LocationOn
                        sx={{ mt: 0.5, mr: 1, color: theme.palette.error.main }}
                      />
                      <Box>
                        <Typography variant="subtitle2" color="text.secondary">
                          Nơi giao hàng
                        ...
                        <Typography variant="body1" sx={{ fontWeight: 500 }}>
                          {trackingData.orderDetails[0].deliveryLocation}
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                          {dayjs(
                            trackingData.orderDetails[0].deliveryDate
                          ).format("MMM D, YYYY")}
                        </Typography>
                      </Box>
                    </Box>
        ...
        {!loading && !trackingData && !error && (
          ...
          >
            ...
            <Typography variant="h6">
              Vui lòng điền mã vận chuyển của bạn để theo dõi đơn hàng.
            </Typography>
          </Box>
        )}
      </Paper>
